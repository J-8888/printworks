...
- Drop `--use-picture` from the workflow: stage-3 per-page comp generation no longer adds the flag, and stage-2 follow-up assets keep using plain commands. Aligns with the create-image v1.2.0 CLI cleanup.
...
- Require follow-up page and asset generation after user selection to use `--use-picture` with page/asset-specific `--title`.
