...
When the product is a **software / SaaS landing page** (or any UI product whose visual quality bar is set by sites like Stripe / Linear / Notion / Vercel / Framer / Obvious / Arc / Raycast), pure text-to-image `generate` reliably produces generic "AI imagined SaaS" comps. The fix is **reference anchoring**: every comp is a `verdent-image edit` remix of a real, hand-picked landing page screenshot.
...
Skip (use normal text generate) only for: games, consumer mobile apps with no web equivalent, content sites (news, magazine, blog), e-commerce.

## Hard rules (override `style-exploration.md` defaults for software class)

1. **Use `verdent-image edit`, not `generate`.** Each of the 4 comps is anchored to one real reference screenshot.
2. **Collect references first.** Before any image call, secure 4 reference screenshots (one per direction). Either:
   ...
   - If user gave none, ask: "Pick 1–4 sites you like (Linear, Stripe, Obvious, Vercel, Framer, Notion, Arc, Raycast, etc.), or I'll choose four high-bar references for you." Wait or pick four canonical ones.
...
6. **Diversity contract for the 4 comps.** The four references must collectively span at least three of:
...
Use these when the user does not name sites. Pick four that span the diversity axes above.
...
|---|---|---|
| Using `generate` with abstract style words | Produces "AI-imagined SaaS" — generic, immediately spotted. | Use `edit` against a real reference. |
| All 4 comps anchored to the same reference | Differences become palette-only; user has no real choice. | Each comp from a different reference, spanning ≥3 diversity axes. |
