...
Goal: give the user **four genuinely different design directions** for the product's primary screen (landing hero / app home / etc.) so they can pick a direction with confidence.

## Tell the user the time estimate first

Before you start generating the four design directions, tell the user — in plain language and in their own conversation language — that producing the design directions is expected to take **about 5–10 minutes**. Phrase it as an estimate (e.g. "around 5–10 minutes" / "around 5–10 minutes"). Do not use the words "stage", "phase", or any internal step numbers.
...
8. Use `verdent-image generate` (not `edit`) unless the user supplied references.
9. **Run four independent `--n 1` commands in parallel.** Stage-1 exploration no longer uses one batched multi-output command. In the same model response, output **4 separate ordinary `bash` tool calls** that each run `verdent-image generate --use-styles --title "<direction name>" --palette "<background>,<primary/ink>,<accent>" --n 1`; ts-agent automatically executes the four bash calls in parallel, so every comp is generated independently and concurrently. Keep `--palette` for CLI/workflow metadata, and also put the exact same three-color system directly in each prompt's COLOR SYSTEM section.
   - Use only ordinary `bash` tool calls here: no poll flow, no `session_id`, and no `operation='output'`.

10. **Each comp commits to one three-color palette in both places.** Give each of the four directions a clear `[background, primary/ink, accent]` color system of 3 `#RRGGBB` values. Pass those colors through `--palette` for CLI/workflow metadata, and express the same colors directly in that comp's prompt body. The palette is just 1 main color + 2 supporting colors. Keep the three colors clearly different from each other — no other constraints. Do not rely only on the separate CLI palette argument; the image model must also see the hex values and usage instructions in the prompt text itself. This palette is a coloring instruction — how the interface is colored — and may only appear through the actual colors of UI elements such as background, text, buttons, cards, and accents. Absolutely do NOT render the palette itself anywhere in the generated image: not inside the UI, not in display-board whitespace, not along canvas edges, not in outer margins around the design, and not beside the mockup. No color swatches, color chips, swatch strips, palette legends, sample color blocks, hex value labels, or any other visible palette reference may appear anywhere in the image. A common violation is placing three vertical color blocks with hex labels on the right side / side edge of the canvas and pretending it is "outside the interface" — this is still forbidden. The palette is used through UI colors, not drawn or labeled.

## Presentation (presentation is a variable, not a fixed template)

You are acting as a world-class digital product design director. There is **no fixed framing**. Choose the composition and presentation that flatters each direction. Forcing all four comps into the identical framing is the #1 cause of comps that "all look the same with different colors."

**Vary the presentation across the four comps** — use at least two clearly different presentation modes so the four never read as one template. How you present each comp is entirely free: pick whatever flatters that direction. The presentation mode is an open creative choice, not a menu. For example (not an exhaustive list): a **display board** (scaled-down mockup on a clean solid backdrop), a **full-bleed** page filling the frame edge-to-edge, a **cropped detail** zooming the hero + one key section, multi-screen layouts, grid showcases, or any other mode that suits the direction.
...
Copy the four returned images into `<project-root>/verdent-design/stage1/` with names `comp-1-{style}.png` … `comp-4-{style}.png`. Keep the original `create-image` output files untouched; do not rely on the global `create-image` cache as the only copy.

## Presenting to the user

Show the four comps with **one short design rationale each** — what era, what taste, who it's for. Do not pick a "winner" yourself. Ask the user to pick one number.

Use plain language. Do not say "stage 1" or "phase 1" to the user.

**Model recommendation (always include here).** When you present the four comps and ask the user to pick a number, also include a short recommendation that they use **Claude Opus 4.8 / 4.7 / 4.6** for development, so the frontend restoration stays high-fidelity. Write this recommendation in the user's own language (match the language they are using in the conversation). Keep it to one friendly sentence, e.g. "We recommend using Claude Opus 4.8 / 4.7 / 4.6 for development to ensure high-quality frontend restoration."
...
If the user does not explicitly choose, ask them to pick one number.

## Common mistakes

| Mistake | Why it fails | Fix |
|---|---|---|
| Generating 1–2 comps "to save time" | The user cannot pick a direction without contrast. | Always exactly 4. |
...
| Using one batched multi-output command or describing multiple styles in one prompt | Model collages styles, repeats one layout, or loses style-card alignment. | In one response, emit 4 separate ordinary `bash` tool calls in parallel, each `verdent-image generate --use-styles --title "<direction name>" --n 1` and one family only; no poll/session flow. |
...
| Four directions all read as the same design family | Variants of one family (e.g. 4 flavors of minimalism) give the user no real choice. | Spread the four across clearly different directions (e.g. Minimalism / Futuristic Tech / Japanese editorial / Swiss / Brutalist / Memphis / Y2K / Skeuomorphic / Hand-drawn), or any other genuinely contrasting set. |
...
| All four comps use the identical framing | Same presentation = comps read as one template with palette swaps. | Vary presentation: use at least two of display-board / full-bleed / cropped across the four. |
