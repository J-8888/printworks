...
function formatPhone(phone: string): string {
  // Strip spaces and leading 0, add UK country code
  const digits = phone.replace(/\s+/g, '').replace(/^0/, '44');
  return digits;
}
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between px-5 py-3 border-b border-[#1e2228] shrink-0">
              ...
              <button
                onClick={onClose}
                className="w-8 h-8 flex items-center justify-center rounded-xl text-[#4a4f5a] hover:text-[#e8e8e8] hover:bg-[#1e2228] transition-colors"
                aria-label="Close"
              >
                <IconX size={16} />
              </button>
